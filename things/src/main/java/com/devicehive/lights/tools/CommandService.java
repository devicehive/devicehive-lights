...
public class CommandService {
    ...
    private Observable<DeviceCommand> observable;
    ...
    private void createCommandSubscription() {
        observable = Observable.create(e -> {
            ...
            device.subscribeCommands(commandFilter, new DeviceCommandsCallback() {
                ...
                public void onFail(FailureData failureData) {
                    e.onError(new Throwable(failureData.getMessage()));
                }
            });
        });
    }
    ...
    private void stop() {
        if (observable != null)
            observable.doOnComplete(() -> System.out.println("onComplete"));
    }
    ...
    private ScheduledFuture<?> getLights(JsonObject jsonObject) {
        int speed = 300;
        Gson gson = new Gson();
        Type listType = new TypeToken<List<ColorParam>>() {
        }.getType();

        final ColorJson colorParam = gson.fromJson(jsonObject.toString(), ColorJson.class);
